...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (empty($_POST["name"]) || empty($_POST["access_number"]) || empty($_POST['card_id']) || empty($_POST['password']) || empty($_POST['balance'])) {
        ...
        try {
            // Insert into users table using student_id as id
            $stmt = $conn->prepare("INSERT INTO users (id, username, password, role, name) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $student_id, $name, $password, $role, $name);
            $stmt->execute();
            ...
            $stmt = $conn->prepare("INSERT INTO students (student_id, name, access_number, card_id, password, balance) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssd", $student_id, $name, $access_number, $card_id, $password, $balance);
            $stmt->execute();
